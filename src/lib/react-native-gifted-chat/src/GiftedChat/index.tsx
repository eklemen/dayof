...
// Custom hook to replace useReanimatedKeyboardAnimation
const useExpoKeyboardAnimation = () => {
...
const ExpoKeyboardProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  return <>{children}</>
}
...
function GiftedChat<TMessage extends IMessage = IMessage> (
  ...
  const keyboard = useExpoKeyboardAnimation()
